...
HttpWithInterceptor createHttpClient() {
  final _client = HttpWithInterceptor.build(
    interceptors: [_CoffeeInterceptor()]
  );
  return _client;
}
...
class _CoffeeInterceptor extends InterceptorContract {
  ...
  Future<void> _handlerUserSection(RequestData data) async {
    ...
    if(await _userBloc.isAuthenticated()) {
      final authenticated = await _userBloc.getCurrentAuthentication();
      data.headers[HttpHeaders.authorizationHeader] = HttpSettings.getAuthorizationHeaderValue(authenticated.token);
    }
